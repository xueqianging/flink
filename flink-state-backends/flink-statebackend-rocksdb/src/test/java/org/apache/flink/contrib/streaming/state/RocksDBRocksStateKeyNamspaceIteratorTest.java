...
import org.apache.flink.contrib.streaming.state.iterator.RocksStateKeyNamespaceIterator;
 ...
 */
public class RocksDBRocksStateKeyNamspaceIteratorTest {
	...
	public void testIterator() throws Exception{
		...
		testIteratorHelper(IntSerializer.INSTANCE, StringSerializer.INSTANCE, 128, "ns", i -> i);

		// test for keyGroupPrefixBytes == 1 && ambiguousKeyPossible == true
		testIteratorHelper(StringSerializer.INSTANCE, StringSerializer.INSTANCE, 128, "ns", i -> String.valueOf(i));

		// test for keyGroupPrefixBytes == 2 && ambiguousKeyPossible == false
		testIteratorHelper(IntSerializer.INSTANCE, StringSerializer.INSTANCE, 256, "ns", i -> i);

		// test for keyGroupPrefixBytes == 2 && ambiguousKeyPossible == true
		testIteratorHelper(StringSerializer.INSTANCE, StringSerializer.INSTANCE, 256, "ns", i -> String.valueOf(i));
	}

	<K, N> void testIteratorHelper(
		...
		try {
			...
			try (
				...
				RocksStateKeyNamespaceIterator<K, N> iteratorWrapper =
					new RocksStateKeyNamespaceIterator<>(
						iterator,
						testStateName,
						keySerializer,
						namespaceSerializer,
				...
				while (iteratorWrapper.hasNext()) {
					Tuple2<K, N> tuple = iteratorWrapper.next();
					Assert.assertEquals(namespace, tuple.f1);
					fetchedKeys.add(Integer.parseInt(tuple.f0.toString()));
				}
