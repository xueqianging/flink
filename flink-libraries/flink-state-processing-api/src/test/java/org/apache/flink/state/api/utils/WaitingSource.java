...
public class WaitingSource<T> extends RichSourceFunction<T> implements ResultTypeQueryable<T> {
	...
	private static final Map<String, InitializationGuard> guards = new HashMap<>();
	...
	public WaitingSource(SourceFunction<T> source, TypeInformation<T> returnType) {
		...
		guards.put(guardId, new InitializationGuard());
		this.running = true;
	}
	...
	public void run(SourceContext<T> ctx) throws Exception {
		try  (InitializationGuard ignore = guards.get(guardId)){
			source.run(ctx);
		}
	...
	public void awaitSource() throws RuntimeException {
		guards.get(guardId).awaitInitialization();
	}
